		...
		begin tran TC
			begin try
				...
				select B.ProductID from (
					(select * from INSTANCIA3.AW_Production.Production.Product) B
					inner join
					(select * from INSTANCIA3.AW_Production.Production.ProductSubcategory) C
...
exec sp_Ejercicioe @Id = 43659, @PId = 776, @cant = 10
...
select Sal  from Sales.SalesOrderHeader
exec sp_Ejerciciof @Ido = 43659, @me = 1
go
